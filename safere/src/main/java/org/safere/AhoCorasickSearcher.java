...
final class AhoCorasickSearcher implements Serializable {
   ...
   */
  int findNext(CharSequence text, int start) {
    int state = 0;
    int len = text.length();
    int bestStart = -1;
    ...
    if (isAsciiOnly) {
      if (caseInsensitive) {
        for (int i = start; i < len; i++) {
          char c = Ascii.toLowerCase(text.charAt(i));
          ...
          if (canReturnBestStart(bestStart, i, start)) {
            return bestStart;
          }
        }
      } else {
      ...
      for (int i = start; i < len; i++) {
        char c = caseInsensitive ? Ascii.toLowerCase(text.charAt(i)) : text.charAt(i);
        if (c < 128) {
          state = transitions[(state << 7) | c];
        } else {
          Integer next =
              nonAsciiTransitions != null
                  ? nonAsciiTransitions.get(((long) state << 32) | c)
                  : null;
          if (next != null) {
            state = next;
          } else {
            ...
            state =
                (next != null)
                    ? next
                    : (f == 0 ? nonAsciiTransitions.getOrDefault((long) c, 0) : 0);
          }
        }
